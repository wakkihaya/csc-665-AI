...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        if position in self.corners:
            if position not in holdcorners:
                holdcorners.append(position)
            return len(holdcorners) == 4
        else:
            return False
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            nextState = (nextx,nexty)
            if not hitsWall:
                sucCorners = list(holdCorners)
                if nextState in self.corners:
                    if nextState not in sucCorners:
                        sucCorners.append(nextState)
                successor = ((nextState, sucCorners),action,1)
...
def foodHeuristic(state, problem):
    ...
    for i in range(foodCount):
        for j in range(foodCount - i - 1):
            dis = mazeDistance(foodList[i], foodList[j + 1], gs)
            if dis > max_dis:
                max_dis = dis
                furthest1 = foodList[i]
                furthest2 = foodList[j + 1]
                part1 = mazeDistance(position, furthest1, gs)
                part2 = mazeDistance(position, furthest2, gs)
                if part1 > part2:
                    part_max_dis = part2
                else:
                    part_max_dis = part1
